...
### Способ 1: С указанием адреса кошелька (рекомендуется)

```bash
OWNER_ADDRESS=0xВАШ_АДРЕС_КОШЕЛЬКА curl -fsSL https://raw.githubusercontent.com/vi11abajo/mawari-guardian-node/main/install.sh | bash
```

**Замените `0xВАШ_АДРЕС_КОШЕЛЬКА` на ваш реальный адрес!**

### Способ 2: Без адреса (настройка вручную)

```bash
curl -fsSL https://raw.githubusercontent.com/vi11abajo/mawari-guardian-node/main/install.sh | bash
```
...
nano .env
# Замените 0xYourWalletAddressHere на ваш адрес
```
...
- ✅ Настроит адрес кошелька (если указан)
